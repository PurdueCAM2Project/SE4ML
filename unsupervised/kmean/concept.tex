...
\marginnote{Heuristic solutionsThis approach is frequently adopted
  in optimization problems.  For many problems, finding the best
  solution (or solutions) can be computationally difficult because
  there are too many possibilities to check.  Instead of finding the
  best solution, an algorithm is adopted to find ``good enough''
  solutions fast. Such an algorithm usually works well but may find
  really bad solutions for some cases.  These solutions are usually
  called {\it heuristic} solutions.  }
