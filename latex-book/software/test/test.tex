...
\linenumbers
\begin{tt}
  \lstinputlisting{\progpath/software/test/test_intpart2.py}
\end{tt}
\nolinenumbers
...
\linenumbers
\begin{tt}
  \lstinputlisting{\progpath/software/test/fullcover.py}
\end{tt}
\nolinenumbers
...
\linenumbers
\begin{tt}
  \lstinputlisting{\progpath/software/test/.travis.yml}
\end{tt}
\nolinenumbers
...
required.  When changes are pushed, run {\tt pytest} and it
automatically looks for a Python program starting with {\tt test\_}
(such as {\tt test\_intpart} on page \pageref{page:test_intpart2.py}.
Figure~\ref{fig:travis02} shows running {\tt pytest} successfully.  If
the test fails, it will appear red.  You are encouraged to modify the
test, make it fail, and see what a failure looks like.

\begin{figure}[h] \centering
{\includegraphics[width=4.5in]{\thischapterpath/figures/travis02.png}}
\caption{Running pytest successfully in Travis-CI.}
\label{fig:travis02}
\end{figure}
