...
and a complete unit, like a section or a chapter in a book.  The tools
that manage versions are called {\it version control}.  When multiple
people work together, version control becomes essential to ensure
proper coordination.
...
include {\tt github.com} and {\tt bitbucket.org}.  This book uses {\tt
  ...
  github}. The toolset
{\tt git} has many different functions and {\tt github}
...
The authors receive no compensation from {\tt github.com}.}
...
Figure~\ref{fig:github1_2} shows the website of {\tt github} and the
...
is created.  This new version is visible at only the local repository,
not the remote repository (in {\tt github}).  To make the changes
visible in {\tt github}, another command {\tt git push} is needed.
\index{git!push}
...
\begin{figure}[h] \centering
{\includegraphics[width=3.8in]{\thischapterpath/figures/gitpush.png}}
...
collaborators to share and exchange.  Adding collaborators would be
...
methods implementing a feature with different versions. Each method
can be a new version or even several versions.  As long as the
...
and which to discard.
  ...
  \index{git!pull} \index{git!push} A few general rules can reduce the
  chances of conflicts: First, communicate and coordinate with
  collaborators often.  Second, do {\tt git pull} and {\tt git push}
  often so that conflicts can be discovered earlier when only a few
  lines of conflicts exist.  In order to do {\tt git push} often, it
  is imperative to focus on one specific problem (e.g., adding one
  feature, or fixing one bug) at any moment, finish the work, and then
  push it.
...
{\it release versions}).
 ...
 \marginnote{It is common restricting the modification of the master branch to only the project leader. }
...
A common mistake among beginning {\tt git} users is that they do not
merge branches. They keep changing their own branches.  They want to
show to their collaborators that they are contributing by frequently
pushing improvements to the repositories.  However, if the
improvements stay in the branches that are not merged, these
improvements are not actually useful.  Most branches should have short
lives: Each branch is created for one specific purpose.  It is
documented, developed, tested, committed, merged, and then deleted.
...
\index{github!issue}
  ...
  Pull requests are restricted to the users that have write
  permissions.  Issues can be raised by people that have no write
  permissions.  Figure~\ref{fig:githubissue} shows an example of an
  issue of a possible error in this book.

\begin{figure}[h] \centering
{\includegraphics[width=4.5in]{\thischapterpath/figures/githubissue2.png}}
\caption{Issue is another way to communicate.
}
\label{fig:githubissue}
\end{figure}
